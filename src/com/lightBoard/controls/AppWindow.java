...
public class AppWindow
{
	...
	private interface ExecuteMe{ void execute(); };
...
//				FormSpecs.RELATED_GAP_ROWSPEC}));
	...
	{
		ExecuteMe speedExec = () -> {if (txtSpeed.getText().length() > 0)
			mControls.setRepeatDelay(Integer.parseInt(txtSpeed.getText()));};
//			txtSpeed.seton
		
		ExecuteMe smoothnessExec = () -> {if (txtSmoothness.getText().length() > 0)
			mControls.setSmoothness(Double.parseDouble(txtSmoothness.getText()));};
		
		ExecuteMe tailLengthExec = () -> {if (txtTailLength.getText().length() > 0)
			mControls.setBufferSize(Integer.parseInt(txtTailLength.getText()));};
		
		ExecuteMe tailWidthExec = () -> {if (txtTailThickness.getText().length() > 0)
